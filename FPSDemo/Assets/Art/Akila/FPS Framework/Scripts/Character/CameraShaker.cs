...
namespace Akila.FPSFramework
{
    ...
    public class CameraShaker : MonoBehaviour
    {
        ...
        [Separator]
        public float defaultRoughness = 1;

        [Range(0, 10)]
        public float defaultFadeInTime = 0.01f;
        [Range(0, 10)]
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                {
                    currentShakePosition += FPSFrameworkCore.MultiplyVectors(profile.Result(), profile.Position);
                    currentShakeRotation += FPSFrameworkCore.MultiplyVectors(profile.Result(), profile.Rotation);
                }
            }
        ...
        public CameraShakeProfile Shake(float magnitude)
        {
            return Shake(magnitude, defaultRoughness, defaultFadeInTime, defaultFadeOutTime);
        }

        public CameraShakeProfile Shake(float magnitude, float roughness)
        {
            return Shake(magnitude, roughness, defaultFadeInTime, defaultFadeOutTime);
        }
